...
- **Configuration Hassle**: All your instance-specific configurations need to be rebuilt
- **Testing Impact**: Having to recreate and revalidate everything disrupts the testing timeline

This wasn't just annoying – it was threatening our project deadline. We needed a clean way to add storage without rebuilding the entire infrastructure.
...
With happy mind i ran *`terraform plan`* and to my surprise i see the terraform plan is showing the instance replacement 😲
...
None of these felt like clean solutions initially. But then I found a workaround that actually worked, shared by a community member on the GitHub issue.

## The Possible Workaround Shared by a Community Member

While searching for solutions, I found a detailed workaround shared by a community member named [HeikoMR](https://github.com/HeikoMR) on the GitHub issue (https://github.com/terraform-aws-modules/terraform-aws-ec2-instance/issues/401#issuecomment-2325773187).
...
State manipulation can be risky and error-prone, especially in team environments where multiple people work with the same infrastructure. I decided this approach felt too fragile for our project requirements and continued exploring for cleaner solutions.
...
The fix addressed the underlying behavior that was causing unnecessary instance replacements when adding EBS volumes. After upgrading to the new provider version, the whole workaround process became unnecessary – you could finally add additional EBS volumes to existing EC2 instances the way it should have worked from the beginning.

Here's what the proper solution looked like:

1. **Update the AWS Provider**: Bump the AWS provider version to 6.0.0 or later in your Terraform configuration
...
## Terraform Code Sample For Practice

If you would like ty try this on your lab environment here are example terraform code 
